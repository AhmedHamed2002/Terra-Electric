    ...
    <section className="py-16 md:py-24 bg-gray-50/10 w-full">
      <div className="container-fluid md:px-10 px-2 mx-auto text-center">
        <FadeIn direction="up" delay={0.1}>
          <p className="text-[#0369a1] font-medium mb-3 uppercase tracking-wider text-sm">
        ...
        <FadeIn direction="up" delay={0.2}>
          <p className="text-gray-600 max-w-3xl mx-auto mb-16 text-sm md:text-base">
            We never ask you to send money to strangers in China. Our team
            inspects every vehicle in person and sends you photos and video before
            you pay anything.
          </p>
        </FadeIn>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 px-3 gap-5 md:gap-4 max-w-7xl mx-auto">
          {steps.map((step, index) => (
            <FadeIn key={index} direction="up" delay={0.1 * index} className="h-full">
              <div
                className={`h-full p-8 group hover:bg-[#176a88]! hover:text-white! flex flex-col items-center justify-center transition-all duration-300 ${
                  step.active
              ...
              >
                <span
                  className={`text-4xl font-bold mb-4 ${
                    step.active ? "text-[#38bdf8]" : "text-gray-500"
                  }`}
                >
                  {step.number}
                </span>
                ...
                <p
                  className={`text-sm text-center group-hover:text-gray-200! ${
                    step.active ? "text-gray-300" : "text-gray-600"
                  }`}
                >
                  {step.description}
                </p>
